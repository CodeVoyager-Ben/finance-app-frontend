...
export default function AssetAllocationChart({ byAssetType, totalMarketValue, hidden }) {
  ...
  const option = {
    tooltip: {
      trigger: 'item',
      formatter: hidden
        ? (p) => `${p.name}<br/>市值: ****<br/>占比: ${p.percent.toFixed(1)}%`
        : (p) => `${p.name}<br/>市值: ${Number(p.value).toFixed(2)}<br/>占比: ${p.percent.toFixed(1)}%`,
    },
    series: [{
      type: 'pie',
      radius: ['50%', '80%'],
    ...
    graphic: [{
      ...
      left: 'center',
      top: '40%',
      style: { text: hidden ? '****' : total.toFixed(2), fontSize: 14, fontWeight: 'bold', textAlign: 'center', fill: '#333' },
    }, {
      type: 'text',
      left: 'center',
      top: '52%',
