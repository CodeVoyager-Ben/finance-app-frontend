...
export default function InvestmentCalendar() {
  ...
  const detailColumns = [
    ...
    },
    {
      ...
      render: (v) => <span style={{ color: plColor(v), fontWeight: 600 }}>{formatMoney(v)}</span>,
    },
    {
      title: '日盈亏%', dataIndex: 'daily_pl_pct', width: 90, align: 'right',
      render: (v) => <span style={{ color: plColor(v) }}>{formatPct(v)}</span>,
    },
    {
              ...
              >
                ...
                {hasData && dailyPlPct !== 0 && (
                  <div style={{
                    ...
                    {dailyPlPct > 0 ? '+' : ''}{Number(dailyPlPct).toFixed(2)}%
                  </div>
                )}
                {hasData && dailyPl === 0 && (
          ...
          <div style={{ display: 'flex', gap: 32, marginTop: 16, paddingTop: 14, borderTop: '1px solid rgba(255,255,255,0.15)' }}>
            <div>
              ...
              <div style={{ fontSize: 22, fontWeight: 700, color: plColor(parseFloat(selectedSummary?.daily_pl || 0)) }}>
                {selectedSummary && parseFloat(selectedSummary.daily_pl) > 0 ? '+' : ''}¥{formatMoney(selectedSummary?.daily_pl || 0)}
              </div>
            </div>
            <div>
              ...
              <div style={{ fontSize: 22, fontWeight: 700, color: plColor(parseFloat(selectedSummary?.total_pl || 0)) }}>
                {selectedSummary && parseFloat(selectedSummary.total_pl) > 0 ? '+' : ''}¥{formatMoney(selectedSummary?.total_pl || 0)}
              </div>
            </div>
          </div>
        </div>
