...
export default function HoldingsTable({ holdings, loading, onViewDetail, onSell, hidden }) {
  const columns = [
    {
    ...
    },
    {
      ...
      render: (v) => <span style={{ color: plColor(Number(v)), fontWeight: 600 }}>{hidden ? '****' : formatPct(v)}</span>,
    },
    {
      title: '年化', dataIndex: 'annualized_return', width: 80, align: 'right',
      render: (v) => <span style={{ color: plColor(Number(v)) }}>{hidden ? '****' : formatPct(v)}</span>,
    },
    {
