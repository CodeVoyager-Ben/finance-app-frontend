...
export default function DashboardStats({ dashboard, loading, hidden }) {
  ...
  const stats = [
    {
      title: '总市值',
      value: mask(fmt(dashboard.total_market_value)),
      icon: <FundOutlined />,
      style: gradientStyle('#1677ff', '#4096ff'),
    },
    {
      title: '总成本',
      value: mask(fmt(dashboard.total_cost)),
      icon: <DollarOutlined />,
      style: gradientStyle('#722ed1', '#9254de'),
    },
    {
      title: '总盈亏',
      value: mask(`${pl >= 0 ? '+' : ''}${fmt(pl)}`),
      subValue: hidden ? '****' : formatPct(dashboard.total_profit_loss_pct),
      icon: pl >= 0 ? <RiseOutlined /> : <FallOutlined />,
      style: gradientStyle(pl >= 0 ? '#f5222d' : '#52c41a', pl >= 0 ? '#ff7875' : '#95de64'),
    },
    {
      title: '今日盈亏',
      value: mask(`${dailyPl >= 0 ? '+' : ''}${fmt(dailyPl)}`),
      subValue: hidden ? '****' : formatPct(dashboard.total_daily_pl_pct),
      icon: <StockOutlined />,
      style: gradientStyle(dailyPl >= 0 ? '#fa541c' : '#13c2c2', dailyPl >= 0 ? '#ff7a45' : '#36cfc9'),
    },
    {
      title: '累计分红',
      value: mask(fmt(dashboard.total_dividend_income)),
      icon: <TrophyOutlined />,
