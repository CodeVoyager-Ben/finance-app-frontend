...
export default function SellModal({ open, holding, onCancel, onOk, loading }) {
    ...
    >
      <Descriptions column={2} size="small" bordered style={{ marginBottom: 16 }}>
        <Descriptions.Item label="可卖数量">
          <Text strong>{maxQty.toFixed(2)}</Text>
        </Descriptions.Item>
        <Descriptions.Item label="成本价">{avgCost.toFixed(4)}</Descriptions.Item>
        <Descriptions.Item label="持仓成本">{formatMoney(avgCost * maxQty)}</Descriptions.Item>
      ...
      {fees && price > 0 && qty > 0 && (
        <>
          ...
          <Descriptions column={2} size="small" bordered>
            <Descriptions.Item label="卖出金额">{formatMoney(amount)}</Descriptions.Item>
            <Descriptions.Item label="佣金 (万2.5)">{fees.commission.toFixed(2)}</Descriptions.Item>
            <Descriptions.Item label="印花税 (0.05%)">{fees.stampDuty.toFixed(2)}</Descriptions.Item>
            <Descriptions.Item label="过户费 (0.001%)">{fees.transferFee.toFixed(2)}</Descriptions.Item>
            <Descriptions.Item label="总手续费">
              <Text type="danger">{fees.total.toFixed(2)}</Text>
            </Descriptions.Item>
            <Descriptions.Item label="净到账">{formatMoney(netProceeds)}</Descriptions.Item>
            <Descriptions.Item label="卖出盈亏" span={2}>
              <span style={{ color: plColor(profitLoss), fontWeight: 700, fontSize: 16 }}>
                ...
                <span style={{ fontSize: 12, marginLeft: 8, fontWeight: 400 }}>
