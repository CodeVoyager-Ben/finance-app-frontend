...
export default function HoldingDetailDrawer({ holding, open, onClose, transactions, hidden }) {
  ...
  return (
    ...
    >
      <Descriptions column={2} bordered size="small">
        ...
        <Descriptions.Item label="数量">{m(Number(holding.quantity).toFixed(2))}</Descriptions.Item>
        <Descriptions.Item label="成本价">{m(Number(holding.avg_cost).toFixed(4))}</Descriptions.Item>
        <Descriptions.Item label="现价">{m(Number(holding.current_price).toFixed(4))}</Descriptions.Item>
        <Descriptions.Item label="市值">{m(formatMoney(holding.market_value))}</Descriptions.Item>
        <Descriptions.Item label="日盈亏">
          <span style={{ color: plColor(dpl), fontWeight: 600 }}>
            {m(`${dpl >= 0 ? '+' : ''}${formatMoney(dpl)}`)} ({m(formatPct(holding.daily_profit_loss_pct))})
          </span>
        </Descriptions.Item>
        <Descriptions.Item label="总盈亏">
          <span style={{ color: plColor(pl), fontWeight: 600 }}>
            {m(`${pl >= 0 ? '+' : ''}${formatMoney(pl)}`)} ({m(formatPct(holding.profit_loss_pct))})
          </span>
        </Descriptions.Item>
        <Descriptions.Item label="总回报率">
          <span style={{ color: plColor(Number(holding.total_return_rate)), fontWeight: 600 }}>
            {m(formatPct(holding.total_return_rate))}
          </span>
        </Descriptions.Item>
        <Descriptions.Item label="年化收益">
          <span style={{ color: plColor(Number(holding.annualized_return)) }}>
            {m(formatPct(holding.annualized_return))}
          </span>
        </Descriptions.Item>
        ...
        <Descriptions.Item label="日均成本">
          {m(holding.daily_avg_cost > 0 ? formatMoney(holding.daily_avg_cost) : '-')}
        </Descriptions.Item>
        <Descriptions.Item label="累计分红">
          {m(formatMoney(holding.accumulated_dividend))}
        </Descriptions.Item>
      ...
      {Number(holding.quantity) > 0 && Number(holding.current_price) > 0 && (() => {
        ...
        return (
          <>
            ...
            <Descriptions column={2} bordered size="small">
              <Descriptions.Item label="卖出金额">{m(formatMoney(fees.trade_amount))}</Descriptions.Item>
              <Descriptions.Item label="佣金 (万2.5)">{m(fees.commission)}</Descriptions.Item>
              <Descriptions.Item label="印花税 (0.05%)">{m(fees.stamp_duty)}</Descriptions.Item>
              <Descriptions.Item label="过户费 (0.001%)">{m(fees.transfer_fee)}</Descriptions.Item>
              <Descriptions.Item label="总费用">
                <span style={{ color: '#ff4d4f' }}>{m(fees.total_fees)}</span>
              </Descriptions.Item>
              <Descriptions.Item label="净到账">{m(formatMoney(fees.net_proceeds))}</Descriptions.Item>
              <Descriptions.Item label="卖出净盈亏" span={2}>
                <span style={{ color: plColor(sellPl), fontWeight: 600 }}>
                  {m(`${sellPl >= 0 ? '+' : ''}${formatMoney(sellPl)}`)}
                </span>
              </Descriptions.Item>
            </Descriptions>
          </>
        ...
        <>
          ...
          <Table
            ...
            columns={[
              ...
              { title: '收盘价', dataIndex: 'close_price', width: 90, align: 'right', render: (v) => hidden ? '****' : Number(v).toFixed(2) },
              { title: '数量', dataIndex: 'quantity', width: 80, align: 'right', render: (v) => hidden ? '****' : Number(v).toFixed(2) },
              { title: '市值', dataIndex: 'market_value', width: 100, align: 'right', render: (v) => hidden ? '****' : formatMoney(v) },
              {
      ...
      <Table
        ...
        columns={[
          ...
          { title: '数量', dataIndex: 'quantity', width: 80, align: 'right', render: (v) => hidden ? '****' : Number(v).toFixed(2) },
          { title: '价格', dataIndex: 'price', width: 80, align: 'right', render: (v) => hidden ? '****' : Number(v).toFixed(4) },
          { title: '手续费', dataIndex: 'fee', width: 80, align: 'right', render: (v) => hidden ? '****' : Number(v || 0).toFixed(2) },
          {
