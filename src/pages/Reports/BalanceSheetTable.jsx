...
export default function BalanceSheetTable({ assets, liabilities, netWorth }) {
        ...
        <Col xs={24} lg={16}>
          ...
          <Card size="small" title="现金及银行" style={{ marginBottom: 12 }}
            ...
            <Table
              ...
              columns={[
                ...
                { title: '类型', dataIndex: 'type', key: 'type' },
                {
                  ...
                  render: v => <Text strong style={{ color: '#52c41a' }}>¥{parseFloat(v).toLocaleString('zh-CN', { minimumFractionDigits: 2 })}</Text>,
                },
              ]}
            />
            ...
            <div style={{ textAlign: 'right' }}>
              ...
              <Text strong style={{ fontSize: 15, color: '#52c41a' }}>
                ¥{parseFloat(assets.cash.total).toLocaleString('zh-CN', { minimumFractionDigits: 2 })}
              </Text>
            </div>
          </Card>
            ...
            <Card size="small" title="证券账户余额" style={{ marginBottom: 12 }}
              ...
              <div style={{ textAlign: 'right' }}>
                ...
                <Text strong style={{ fontSize: 15, color: '#1677ff' }}>
                  ¥{parseFloat(assets.invest_balance).toLocaleString('zh-CN', { minimumFractionDigits: 2 })}
                </Text>
              </div>
            </Card>
          )}
            ...
            <Card size="small" title="投资持仓" styles={{ body: { padding: 12 } }}>
              <Table
                ...
                columns={[
                  ...
                  { title: '类型', dataIndex: 'type', key: 'type' },
                  {
                    ...
                    render: v => <Text strong>¥{parseFloat(v).toLocaleString('zh-CN', { minimumFractionDigits: 2 })}</Text>,
                  },
                  {
              ...
              <div style={{ textAlign: 'right' }}>
                ...
                <Text strong style={{ fontSize: 15 }}>
                  ¥{parseFloat(assets.investments.total).toLocaleString('zh-CN', { minimumFractionDigits: 2 })}
                </Text>
              </div>
            </Card>
          )}
          ...
          {receivables.items.length > 0 && (
            <Card size="small" title="应收款项（借出未收回）" style={{ marginTop: 12 }}
              ...
              <div style={{ textAlign: 'right' }}>
                <Text>小计: </Text>
                <Text strong style={{ fontSize: 15, color: '#1677ff' }}>¥{fmt(receivables.total)}</Text>
              </div>
            </Card>
          )}
              ...
              <Card size="small" title="透支账户" style={{ marginBottom: 12 }}
                  ...
                  columns={[
                    ...
                    { title: '类型', dataIndex: 'type', key: 'type', render: v => v || '-' },
                    {
                      ...
                      render: v => <Text strong style={{ color: '#ff4d4f' }}>
                        ¥{parseFloat(v).toLocaleString('zh-CN', { minimumFractionDigits: 2 })}
                      </Text>,
                    },
                  ]}
                />
                ...
                <div style={{ textAlign: 'right' }}>
                  ...
                  <Text strong style={{ fontSize: 15, color: '#ff4d4f' }}>
                    ¥{parseFloat(liabilities.total - payables.total).toLocaleString('zh-CN', { minimumFractionDigits: 2 })}
                  </Text>
                </div>
              </Card>
                ...
                <Card size="small" title="应付款项（借入未归还）" style={{ marginBottom: 12 }}
                  ...
                  <Table
                    ...
                    columns={[
                      ...
                      {
                        title: '金额', dataIndex: 'amount', key: 'amount', align: 'right',
                        render: v => <Text strong style={{ color: '#ff4d4f' }}>¥{fmt(v)}</Text>,
                      },
                    ]}
                  />
                  <Divider style={{ margin: '8px 0' }} />
                  <div style={{ textAlign: 'right' }}>
                    <Text>小计: </Text>
                    <Text strong style={{ fontSize: 15, color: '#ff4d4f' }}>¥{fmt(payables.total)}</Text>
                  </div>
                </Card>
              )}
            </>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 32 }}>
            <div>
              ...
              <div style={{ fontSize: 22, fontWeight: 700, color: '#69db7c' }}>¥{fmt(detailMeta.total)}</div>
            </div>
            <div>
              ...
              <Card key={record.id} size="small" style={{ marginBottom: 12 }}
                ...
                <Descriptions size="small" column={{ xs: 1, sm: 2 }} style={{ marginBottom: 0 }}>
                  <Descriptions.Item label="金额">
                    <Text strong style={{ fontSize: 15 }}>¥{fmt(record.amount)}</Text>
                  </Descriptions.Item>
                  <Descriptions.Item label="已还">
                    <Text style={{ color: '#52c41a' }}>¥{fmt(record.repaid_amount)}</Text>
                  </Descriptions.Item>
