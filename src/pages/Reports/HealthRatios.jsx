...
export default function HealthRatios({ ratios, netWorthChange, hidden }) {
    ...
    <Card title={
      ...
      <Row gutter={[16, 16]}>
        <Col xs={12} sm={6}>
          <RatioCard
            label="资产负债率"
            value={hidden ? 0 : ratios.debt_ratio}
            max={0.8}
        ...
        <Col xs={12} sm={6}>
          <RatioCard
            label="流动性比率"
            value={hidden ? 0 : Math.min(ratios.current_ratio, 20)}
            max={10}
            format={hidden ? () => '****' : v => ratios.current_ratio >= 999 ? '无负债' : v.toFixed(2)}
        ...
        <Col xs={12} sm={6}>
          <RatioCard
            label="月储蓄率"
            value={hidden ? 0 : ratios.savings_ratio}
            max={1}
            format={hidden ? () => '****' : v => `${(v * 100).toFixed(1)}%`}
        ...
        <Col xs={12} sm={6}>
          <Card size="small" style={{
            ...
            <div style={{ marginTop: 8 }}>
              <Statistic
                value={hidden ? '****' : (netWorthChange?.change || 0)}
                prefix={hidden ? null : (isPositive ? <ArrowUpOutlined /> : <ArrowDownOutlined />)}
                suffix={hidden ? null : `(${netWorthChange?.change_pct > 0 ? '+' : ''}${netWorthChange?.change_pct || 0}%)`}
                valueStyle={{
