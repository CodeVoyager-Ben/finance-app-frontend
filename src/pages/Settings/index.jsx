...
export default function Settings() {
  ...
  const hidden = usePrivacyStore((s) => s.hiddenPages)['settings']
  ...
  const accountColumns = [
    ...
    },
    {
      ...
      render: (v, r) => {
        if (hidden) return <Text strong>****</Text>
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        ...
        <PrivacyToggle pageKey="settings" />
      </div>
              ...
              >
                <Table
                  columns={[
                    ...
                    { title: '预算金额', dataIndex: 'amount', key: 'amount', render: v => hidden ? '****' : `¥${parseFloat(v).toFixed(2)}` },
                    { title: '已花费', dataIndex: 'spent', key: 'spent', render: v => hidden ? '****' : `¥${parseFloat(v).toFixed(2)}` },
                    {
                      title: '状态', key: 'status',
                      render: (_, r) => {
