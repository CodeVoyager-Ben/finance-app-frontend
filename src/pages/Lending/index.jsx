...
export default function Lending() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (v) => <Text strong>{hidden ? '****' : `¥${parseFloat(v).toFixed(2)}`}</Text>,
    },
    {
      ...
      render: (v, r) => <span style={{ color: parseFloat(v) >= parseFloat(r.amount) ? '#52c41a' : '#666' }}>
        {hidden ? '****' : `¥${parseFloat(v).toFixed(2)}`}
      </span>,
    },
    {
      title: '剩余', dataIndex: 'remaining_amount', key: 'remaining_amount', width: 110, align: 'right',
      render: (v) => <Text type="danger" strong>{hidden ? '****' : `¥${parseFloat(v).toFixed(2)}`}</Text>,
    },
    {
  ...
  const repaymentColumns = [
    ...
    { title: '金额', dataIndex: 'amount', key: 'amount', width: 100, align: 'right', render: v => hidden ? '****' : `¥${parseFloat(v).toFixed(2)}` },
    { title: '利息', dataIndex: 'interest', key: 'interest', width: 80, align: 'right', render: v => hidden ? '****' : (v ? `¥${parseFloat(v).toFixed(2)}` : '-') },
    ...
    <div>
      ...
      {summary && (
        <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
          <Col xs={12} sm={6}>
            <Card style={{ background: 'linear-gradient(135deg, #722ed1 0%, #9254de 100%)' }} styles={{ body: { padding: '20px 24px' } }}>
              <Statistic
                title={<span style={{ color: 'rgba(255,255,255,0.85)' }}>总借出</span>}
                value={hidden ? '****' : summary.total_lent}
                prefix={hidden ? '' : '¥'}
          ...
          <Col xs={12} sm={6}>
            <Card style={{ background: 'linear-gradient(135deg, #1677ff 0%, #4096ff 100%)' }} styles={{ body: { padding: '20px 24px' } }}>
              <Statistic
                title={<span style={{ color: 'rgba(255,255,255,0.85)' }}>总借入</span>}
                value={hidden ? '****' : summary.total_borrowed}
                prefix={hidden ? '' : '¥'}
                precision={hidden ? undefined : 2}
                valueStyle={{ color: '#fff', fontWeight: 700 }}
              />
          ...
          <Col xs={12} sm={6}>
            <Card style={{ background: 'linear-gradient(135deg, #fa8c16 0%, #ffa940 100%)' }} styles={{ body: { padding: '20px 24px' } }}>
              <Statistic
                title={<span style={{ color: 'rgba(255,255,255,0.85)' }}>待收回</span>}
                value={hidden ? '****' : summary.total_lent_remaining}
                prefix={hidden ? '' : '¥'}
                precision={hidden ? undefined : 2}
                valueStyle={{ color: '#fff', fontWeight: 700 }}
              />
          ...
          <Col xs={12} sm={6}>
            <Card style={{ background: 'linear-gradient(135deg, #f5222d 0%, #ff4d4f 100%)' }} styles={{ body: { padding: '20px 24px' } }}>
              <Statistic
                title={<span style={{ color: 'rgba(255,255,255,0.85)' }}>待归还</span>}
                value={hidden ? '****' : summary.total_borrowed_remaining}
                prefix={hidden ? '' : '¥'}
                precision={hidden ? undefined : 2}
                valueStyle={{ color: '#fff', fontWeight: 700 }}
              />
          ...
          <>
            <Card size="small" style={{ marginBottom: 16, background: '#f6f6f6' }}>
              <Descriptions column={2} size="small">
                ...
                <Descriptions.Item label="总额">{hidden ? '****' : `¥${parseFloat(selectedRecord.amount).toFixed(2)}`}</Descriptions.Item>
                <Descriptions.Item label="已还">{hidden ? '****' : `¥${parseFloat(selectedRecord.repaid_amount).toFixed(2)}`}</Descriptions.Item>
                <Descriptions.Item label="剩余">
                  <Text type="danger">{hidden ? '****' : `¥${parseFloat(selectedRecord.remaining_amount).toFixed(2)}`}</Text>
                </Descriptions.Item>
              </Descriptions>
            </Card>
        ...
        {selectedRecord && (
          <div>
            <Descriptions column={1} bordered size="small" style={{ marginBottom: 24 }}>
              ...
              <Descriptions.Item label="对方">{selectedRecord.counterparty}</Descriptions.Item>
              <Descriptions.Item label="金额">{hidden ? '****' : `¥${parseFloat(selectedRecord.amount).toFixed(2)}`}</Descriptions.Item>
              <Descriptions.Item label="已还金额">{hidden ? '****' : `¥${parseFloat(selectedRecord.repaid_amount).toFixed(2)}`}</Descriptions.Item>
              <Descriptions.Item label="剩余金额">
                <Text type="danger">{hidden ? '****' : `¥${parseFloat(selectedRecord.remaining_amount).toFixed(2)}`}</Text>
              </Descriptions.Item>
              <Descriptions.Item label="利息">{hidden ? '****' : `¥${parseFloat(selectedRecord.interest_amount).toFixed(2)}`}</Descriptions.Item>
