  ...
  const columns = [
    ...
    { title: '账户', dataIndex: 'account_name', key: 'account', width: 80 },
    {
      ...
      render: (v, r) => <span style={{ color: r.transaction_type === 'income' ? '#52c41a' : '#ff4d4f', fontWeight: 600 }}>
        {r.transaction_type === 'income' ? '+' : '-'}¥{parseFloat(v).toFixed(2)}
      </span>,
    },
                    ...
                    <Card size="small" style={{ borderRadius: 10 }}
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                        <Text type="secondary" style={{ fontSize: 12 }}>
                          已花 ¥{parseFloat(b.spent).toFixed(0)}
                        </Text>
                        <Text style={{ fontSize: 12, color: isOver ? '#ff4d4f' : '#52c41a' }}>
                          {isOver ? `超支 ¥${Math.abs(parseFloat(b.remaining)).toFixed(0)}` : `剩余 ¥${parseFloat(b.remaining).toFixed(0)}`}
                        </Text>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 2 }}>
                        <Text type="secondary" style={{ fontSize: 11 }}>
                          预算 ¥{parseFloat(b.amount).toFixed(0)}
                        </Text>
                      </div>
                    </Card>
                  </Col>
